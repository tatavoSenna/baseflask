...
def generate_token(current_user):
    ...
    try:
        ...
        logging.exception("Could not generate external token")
...
def authorize_token():
    ...
    if token is None:
        return jsonify({"Authorized": False, "Message": "Token is missing"}), 200
    try:
        template_id = authorize_external_token_controller(token)
    except ExceptionWithMsg as e:
        return jsonify({"Authorized": False, "Message": e.msg}), 200
    ...
    if document_template is None:
        return (
            jsonify(
                {
                    "Authorized": False,
                    "Message": "Document Template not found in database",
                }
            ),
            200,
        )
        ...
        jsonify(
            {
                ...
                "Template": DocumentTemplateSerializer(many=False).dump(
                    document_template
                ),
            }
        ),
