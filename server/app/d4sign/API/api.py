...
class D4SignAPI:
    ...
    def __init__(self, config=None, company=None):        
        ...
        self.document_webhook_url = \
            config.get('D4SIGN_API_DOCUMENT_WEBHOOK_URL')

        if company is None:
            self.token = config.get('D4SIGN_API_LAWING_TOKEN')
            self.cryptkey = config.get('D4SIGN_API_LAWING_CRYPTKEY')
            self.hmac_secret = \
                config.get('D4SIGN_API_LAWING_DOCUMENT_WEBHOOK_HMAC_SECRET')
        else:
    ...
    def put_url_credentials(self, url: str = '') -> str:
        return '{}?tokenAPI={}&cryptKey={}'.format(
            url,
            self.token,
            self.cryptkey
        )
    ...
    def get_safe(self, safe_uuid: str = '', safe_name: str = '') -> Dict:
        ...
        try:
            safe, = filter(
                lambda safe: safe[lookup_key] == lookup_val,
                safes
            )
    ...
    def register_document_webhook(self,
                                  document_uuid: str) -> Dict:
        """
    ...
    def register_document_signer(self,
                                 document_uuid: str,
                                 signer_email: str,
                                 act: int = 1,
                                 foreign: int = 0,
